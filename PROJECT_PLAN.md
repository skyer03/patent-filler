...
> 文档版本：v1.0
> 更新日期：2026-08-04  
> 项目状态：M7 统一入口、离线回归与发布包已完成；现场代表性样本需在目标 Windows 环境按验收模板执行，保存/删除仍由用户人工执行
...
本项目不接入网站后端、不修改网站源代码，也不依赖公网服务。真实网页主路线使用企业 Edge 扩展在用户明确操作的当前标签页中按 DOM 预填；扩展只访问经批准的精确内网 Origin，通过本机 Native Messaging 取得已审核字段。最终保存前必须由用户人工检查并确认。
...
- 不逆向或调用未授权网站接口；DOM 预填只由企业 Edge 扩展在批准域名执行，不使用 `<all_urls>`、Cookie、浏览历史或公网权限。
...
| 申请名称 | 发明名称/实用新型名称/外观设计名称 | 自动 | 不从文件名取值 |
| 申请类型 | 证书标题或名称标签 | 自动 | 发明、实用新型、外观设计三选一 |
...
| 发明人表格 | 当前发明人（外观设计证书为“设计人”）列表及原顺序 | 自动后复核 | 名单低置信度时禁止自动填表 |
...
M1–M6 已分别形成证书解析、离线模拟、屏幕识别、M4 草稿流程、M5 现场单步操作和 M6 稳定性基础设施，但当前启动命令仍按阶段分开：`m4` 主要运行离线模拟，`m5` 运行现场识别/单步操作，`m6` 主要管理队列状态。M7 的目标是把这些能力整合为一个用户可以直接启动和使用的工具，不把阶段命令误认为最终产品入口。

交付物：

- 一个统一的桌面启动入口和任务界面，能够选择模拟、只识别、单步和受控批量模式；
- 从 PDF 导入、证书解析、人工校对、人工配置、浏览器绑定、页面识别、用户确认、屏幕填报、动作后回读到最终报告的统一流程；
- 将 M6 任务队列、checkpoint、失败恢复、重试、诊断和 profile/config 版本管理接入统一界面；
- 明确区分 `InMemoryPageAdapter` 离线模拟和 M5 前台屏幕执行器，真实网页操作不能只返回模拟成功；
- 统一显示低置信度、页面异常、失焦、编辑态不符、动态表格异常和回读不一致等安全停机原因；
- 可离线安装的软件包/启动脚本、升级说明、用户操作手册和现场验收记录模板；
- 保留保存、提交、返回、删除以及自动创建下一条记录的人工确认边界；
- M7 端到端测试、离线 50 份回归和现场代表性样本验收记录。

退出标准：

- 用户只需启动统一入口即可完成一份证书从导入到保存前最终人工复核的完整流程；
- 模拟模式的结果不会冒充真实网页操作，真实网页模式必须有窗口绑定、识别、输入和动作后回读证据；
- M6 队列可以从统一界面查看、暂停、恢复和重试，程序异常退出后不会静默跳过或重复执行任务；
- 低置信度、错误页面、窗口失焦、编辑态不符、动态表格异常或字段回读不一致时，当前任务安全停机并保留可脱敏诊断；
- 离线 50 份 golden 回归保持通过；现场至少覆盖短名单、长名单、联合权利人和扫描证书，并逐字段人工复核；
- 程序默认停在最终保存前，用户能够明确看到待确认、已完成、跳过和失败字段；
- 安装包可在目标 Windows 环境离线安装，升级前可备份，失败后可回滚到已验证版本。

M7 暂不默认启用自动创建下一条记录，也不默认允许配置驱动的非证书字段批量填写；这两项仍须取得业务确认并单独增加验收用例。
...
- 证书字段通过 golden 回归；
- 用户可在填报前审阅和修正所有值；
- 用户可以从统一入口完成解析、校对、网页识别、受控填报、验证和保存前复核；
...
| v0.9 | 2026-08-04 | 明确 M6 与最终可用工具之间的差距，新增 M7 产品化整合与发布阶段：统一入口、统一任务流程、真实网页执行器接入、M6 队列接入 GUI、端到端验收和离线安装发布标准。 |
| v1.0 | 2026-08-04 | 完成 M7 统一桌面入口、四种运行模式、M6 队列控制、模拟/真实执行器边界、统一安全停机报告、离线安装升级包、测试操作说明和现场验收记录模板；现场代表性样本需在目标环境执行。 |
| v1.1 | 2026-08-04 | M7 首页简化为绑定页面、上传单份证书、开始更新信息三步；增加公开差异计划、Profile 驱动的屏幕回读、一键逐动作验证、冲突值/低置信度/失焦立即停机和空白/冲突测试页；高级模式折叠保留，保存仍由用户手动执行。 |
| v1.2 | 2026-08-10 | 真实网页主路线由屏幕 OCR/坐标执行切换为企业 Edge DOM 扩展；新增本机已审核任务、Native Messaging、六项基本字段唯一定位/冲突保护/写后回读、脱敏 DOM 校准、精确 Origin 配置及当前用户级安装脚本。屏幕执行器降级为高级诊断和兜底，保存/提交/返回/删除仍禁止自动执行。 |
