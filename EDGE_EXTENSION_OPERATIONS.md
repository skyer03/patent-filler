...
桌面界面默认只发布五个证书基础字段，以及人工明确提供时的“联合申请”。动态权利人表、发明人表、第一发明人选择器和两个合并字段已经实现，但必须先在真实空白页完成 Profile 校准，再显式启用：

```powershell
python -m app dom prepare reviewed.json --include-complex
```

启用前逐项确认：表格本体、新增按钮、行选择器、新行输入框、人员搜索框、候选行、选择按钮和选择后回读区域均只能匹配一个预期目标。表格只允许在现有行与审核草稿前缀一致时追加缺失行；扩展没有删除行路径。人员候选不唯一时直接转人工。
...
5. 完成复杂控件校准后再使用 `--include-complex`，分别验证空表、已有正确前缀、已有错误行、短名单、长名单和人员重名。
6. 模拟扩展断开、本机组件未注册、任务文件损坏、Profile 版本不一致和用户中途停止。
7. 每次都停在保存前，由人工逐字段检查后自行决定是否保存；扩展测试期间不要授权自动保存、提交、删除或新建记录。

通过标准：错误页面和未知版本发送输入次数为零；默认冲突值从不覆盖，覆盖模式必须有桌面人工确认；表格不删除、不改写既有正确行；人员不唯一时停止；结果日志只保存值是否存在、长度和哈希；所有自动流程的保存、提交、返回、删除和新建次数均为零。
